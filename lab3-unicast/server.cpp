...
void readThread(int ConnectFD){
  while(true){
    ...
    std::string send = sizeString(dest2.length()) + dest2 + sizeString(msg2.length()) + msg2;
    
    if(buff.find(dest2) != buff.end())
      write(buff[dest2], send.c_str(), send.length());
